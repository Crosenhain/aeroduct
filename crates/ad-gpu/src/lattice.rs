...
pub const D3Q19: LatticeDef = LatticeDef {
    q: 19,
    directions: &D3Q19_DIRS,
    weights: &D3Q19_WEIGHTS,
};
pub const D3Q27: LatticeDef = LatticeDef {
    q: 27,
    directions: &D3Q27_DIRS,
...
pub fn wgsl_prelude(set: super::types::VelocitySet) -> String {
    ...
    s.push_str(&format!(
        "const C: array<vec3<i32>, {}> = array<vec3<i32>, {}>(\n",
        def.q, def.q
    ));
    ...
    fn check_set(dirs: &[IVec3], weights: &[f32]) {
        ...
        for i in (1..dirs.len()).step_by(2) {
            assert_eq!(
                dirs[i],
                -dirs[i + 1],
                "directions {i} and {} are not opposites",
                i + 1
            );
        ...
        for a in 0..3 {
            ...
            for b in 0..3 {
                ...
                assert!(
                    (m2[a][b] - want).abs() < 1e-6,
                    "second moment wrong at {a},{b}: {m2:?}"
                );
            }
        }
    }
                ...
                for yi in 0..N {
                    for xi in 0..N {
                        ...
                        for i in 1..19usize {
                            ...
                            let stored =
                                ep.store_slot(n_here, i, odd, |d| lin(here + D3Q19_DIRS[d]));
                            ...
                            let loaded =
                                ep.load_slot(n_there, i, !odd, |d| lin(there + D3Q19_DIRS[d]));
                            assert_eq!(
                                stored, loaded,
